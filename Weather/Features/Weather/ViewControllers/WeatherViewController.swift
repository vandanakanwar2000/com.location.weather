...
final class WeatherViewController: UIViewController {
    ...
    private let fadeInAlpha: CGFloat = 0.4
    ...
    @IBAction func moreInfoButtonClicked(_: UIButton) {
    ...
    @IBAction func searchLocation(_: UIButton) {
        locationListActionHandler(Storage().fetch())
    }
    ...
    private func dailyForcastList(detailViewViewModels: [ForecastViewModel]) -> UIStackView {
        let listStackView = stackview(spacing: standardPadding / 2)
    ...
    private func detailList(detailViewViewModels: [DetailViewViewModel]) -> UIStackView {
        let listStackView = stackview(spacing: standardPadding)
    ...
    private func stackview(spacing: CGFloat) -> UIStackView {
        let listStackView = UIStackView(frame: .zero)
        listStackView.spacing = spacing
        listStackView.axis = .vertical
        return listStackView
    }
}
...
extension WeatherViewController: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        tempMax.alpha = 1 - (alphaValue + fadeInAlpha)
        tempMin.alpha = 1 - (alphaValue + fadeInAlpha)
        currentDay.alpha = 1 - (alphaValue + fadeInAlpha)
    }
}
